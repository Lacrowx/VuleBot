...
bot = commands.Bot(command_prefix='!', activity=discord.Activity(type=discord.ActivityType.watching, name="Valentin 24/7 zu"), status=discord.Status.idle)
...
async def test(ctx, response=None):
...
async def video(ctx):
    if (ctx.channel.id == 813582532120674356 or ctx.channel.id == 452731186360614922):
        conn = create_connection(database)
        cur = conn.cursor()
        await ctx.send(f"Hallo, {ctx.message.author.mention}! Gerne schaue ich für dich nach, ob ein neues Video veröffentlicht wurde. Einen Moment.")
